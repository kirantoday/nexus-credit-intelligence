...
| **Current Status** | Milestone 7.5.2 is complete, including a same-day product-definition correction. First pass: root-caused production's stale "Last successful run: Aug 6" display (`get_latest_successful_run` treated any successful mode, including `backfill`, as the watermark/display source) and fixed it with a mode-scoped daily-run boundary; ran a real 2026-08-07 delta (246 new issuers, 1207 new SEC filings, 822 new evidence rows, 356 new alerts, 0 errors, idempotency-proven); found Milestone 15 already satisfied. Second pass (this same day, explicit follow-up direction): the brief's own definition was still too pipeline-centric — corrected to answer "what materially changed since this user last reviewed the brief," a user-relative boundary (`morning_brief_view`, new migration `0012`) with a documented previous-business-day fallback for a first-ever view, pipeline/run counters demoted to a secondary `RunDetails` block, and alerts now issuer-grouped, severity-ranked, and split into `new_developments` vs. `historical_intelligence` by `is_backfill`. A real performance regression was caught and fixed during this pass: the first implementation issued 2 extra DB queries per alert (N+1), which timed out entirely (>50s) against real production volume (~350 alerts) — fixed with batched issuer/universe-membership lookups, bringing a real request down to ~1.7s. TD-018 records the single-shared-timeline (not per-user) interim posture, since no authentication/session infrastructure exists yet (TD-002). Production browser verification of the correction then surfaced a genuinely unresolved intermittent `503` on `POST /api/morning-brief/view` (TD-019) — extensively investigated (response-size cap, TanStack Query retry, then a direct manual retry, each live-tested against production) without a confirmed root cause or a confirmed-working fix; honestly recorded as open rather than claimed resolved, since it does not block the primary `GET /api/morning-brief` read or corrupt any data (self-healing worst case). Full metrics in `BUILD_LOG.md`. |
| **Last Updated** | 2026-08-09 |
| **Current Git Branch** | main |
| **Latest Commit** | `e8a70ff` — fix: replace ineffective TanStack Query mutation retry with direct retry |
...
| TD-018 | The Morning Research Brief's "since you last looked" boundary (`morning_brief_view`, Milestone 7.5.2's correction) is a single shared, append-only timeline, not a per-user one — Nexus has no authentication/session infrastructure yet (TD-002, open; confirmed by direct inspection: `Settings.auth_enabled = False`, no `user` table, no session/cookie middleware, no `Depends(get_current_user)` anywhere, zero frontend `localStorage`/`sessionStorage` usage). A new view is only recorded when the most recent one is more than `MIN_VIEW_GAP` (4 hours, a documented heuristic constant) old, which keeps rapid refresh/reopen idempotent within one working session but means two different analysts using Nexus "at the same time" currently share one boundary rather than each seeing "since I last looked" | Medium | Once real user identity/sessions exist (TD-002's resolution), scope `morning_brief_view` (or its replacement) per user — the `morning_brief_service.py` boundary-resolution logic is already isolated behind `_resolve_period_start`/`record_brief_view`, so this should be a targeted change, not a rewrite | Open (deferred by design — genuinely no user identity to attach to yet, not an oversight) |
| TD-019 | `POST /api/morning-brief/view` intermittently returns `503` from Railway's edge when triggered by a real, fresh browser page load — root cause **not conclusively identified**, despite two rounds of live investigation. First hypothesis (large `GET /api/morning-brief` response, up to 3.5MB on a first-ever view, briefly blocking the single Uvicorn worker's event loop and disrupting the next request on the same reused connection) was disproved by direct re-test: after capping the response to ~730KB (see the now-corrected description below), the exact same `503` still reproduced on a fresh page load with a *tiny* (1106-byte) `GET` response. What is established, live-verified, not guessed: the failure reproduces consistently and only via a genuine browser-triggered fresh page load (confirmed across 4 separate tabs); it has never once reproduced via `curl` (15+ attempts, including true concurrency), a manual same-page `fetch()` call (10+ attempts, including one replaying the exact request sequence and one forcing a fresh CORS preflight), or a client-side (SPA) route change; the deployed JS bundle was confirmed byte-for-byte to construct the request exactly as intended (no build/caching artifact); and every retry — manual or automated — reliably succeeds. The response-size cap (`_ISSUER_DISPLAY_CAP` — see `app/services/morning_brief_service.py`) was kept regardless, since a smaller response is good practice independent of this bug, but it is not the fix. First mitigation attempt (`useMutation({retry: 2})`, TanStack Query's own mutation retry) was itself live-verified to **not actually re-attempt the call** — Resource Timing API confirmed exactly one network request fired regardless of the failure, despite the deployed bundle containing the correct `retry:2,retryDelay:1e3` config — a second real, live-caught surprise in this same investigation. Replaced with a direct, manual retry loop in `recordMorningBriefView` itself (`web/src/api/filingMonitor.ts`, 3 attempts, 1s delay, no TanStack Query involvement) — safe, since `record_brief_view` is idempotent by construction. Deployed and re-verified byte-for-byte in the live bundle (the loop is present exactly as written). **Honest final status, not a claimed fix**: post-deploy re-testing in the live browser still showed only one network request/`503` per real fresh-page-load failure, via both Resource Timing API and repeated checks — whether this means the deployed retry genuinely doesn't re-attempt for real users, or reflects a limitation of the diagnostic tools available in this automated browser session (unable to rule out), was not conclusively resolved; further diagnosis was judged disproportionate to a secondary, self-healing, non-blocking feature. Does not block core functionality: `GET /api/morning-brief` (the primary read) has never failed in any test, live or scripted; a failed `POST /view` only means the "since you last looked" boundary doesn't advance that one visit, self-healing on the next successful call (next session, since `MIN_VIEW_GAP` will have elapsed) | Medium | Root-cause with Railway-side request/edge logs (not available from this environment) — likely candidates still open: Railway edge/proxy behavior specific to this deployment tier, or an interaction with the automated test browser's own extensions/CDP instrumentation rather than a real end-user condition. Separately, confirm (with real end-user traffic or Railway logs) whether the deployed retry loop actually re-attempts — this could not be confirmed from this environment | Open — genuinely unresolved, not merely "mitigated" (`web/src/api/filingMonitor.ts`, `backend/app/services/morning_brief_service.py`) |
