...
| **Current Status** | Milestone 7.5.2 is complete, including two same-day product-definition corrections. First pass: root-caused production's stale "Last successful run: Aug 6" display (`get_latest_successful_run` treated any successful mode, including `backfill`, as the watermark/display source) and fixed it with a mode-scoped daily-run boundary; ran a real 2026-08-07 delta (246 new issuers, 1207 new SEC filings, 822 new evidence rows, 356 new alerts, 0 errors, idempotency-proven); found Milestone 15 already satisfied. Second pass: corrected the brief to a user-relative "since you last viewed it" boundary (`morning_brief_view`) — found live, via production browser verification, to have both a real performance problem (N+1 queries, fixed) and a genuinely unresolved intermittent `503` (TD-019, extensively investigated, never conclusively root-caused). Third pass (same day, explicit follow-up direction): page views were never the right business boundary at all — corrected to "what materially changed during the latest completed business-day research cycle, compared with the preceding one," derived purely from canonical successful daily-run data (a new `research_day` field, sourced from `window_start_date`/`previous_watermark`) plus calendar business-day arithmetic (Mon-Fri, weekends skipped) — opening, refreshing, or revisiting the brief can never change the window; only a new successful daily run completing can. `morning_brief_view` and `POST /api/morning-brief/view` were removed entirely (migration `0013`, TD-018/TD-019 closed by removal, not by building per-user auth or root-causing the `503` — both recorded honestly as closed-by-architecture-change, not silently dropped). Production-verified: the brief now shows "Latest research day: Aug 7, 2026 · Compared with: Aug 6, 2026," unchanged across repeated refreshes. Full metrics in `BUILD_LOG.md`. |
