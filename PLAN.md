...
| Field | Value |
|---|---|
| **Overall Progress** | ~31% — Milestone 5 of 16 complete |
| **Current Milestone** | Milestone 5 — OpenFIGI + FRED adapters, feeding more Credit Universe columns (complete) |
| **Current Status** | Milestone 5 is complete. Both adapters were scoped around a real analyst question rather than integrated for their own sake: OpenFIGI answers "what security is this?", FRED answers "what macroeconomic environment surrounds this credit?" — and the Credit Universe screen is measurably more useful for it. OpenFIGI: a live `/v3/search` call for "APPLE INC" (`marketSecDes=Corp`, filtered to `exchCode=TRACE`) identified five real, specific Apple corporate bonds (real FIGI, real maturity/coupon parsed from OpenFIGI's own `"COUPON MM/DD/YY"` ticker convention) — confirmed live that the free API returns no CUSIP/ISIN at all (Bloomberg/ANNA license those separately), so those stay honestly `None`. These became new `security` rows rather than enriching the existing SEC-aggregate row: attaching one specific bond's FIGI to a row that represents Apple's whole aggregate long-term debt would misattribute the fact, so TD-007 (per-field provenance) stays open by design rather than being forced — each new row is purely OpenFIGI-sourced, so its single `provenance_id` is already fully correct. FRED: real SOFR and ICE BofA US High Yield OAS (`BAMLH0A0HYM2`) series are synced (latest 10 observations each, not a bulk historical backfill — TD-009), with every observation's own `provenance`/`raw_provider_payload` preserved and FRED's real `"."` missing-observation marker filtered out rather than persisted as a fabricated null fact. Credit Universe gained a "Current Benchmark Rate" column (a plain reported FRED fact re-displayed per matching row, e.g. every SOFR-referencing loan — not blended with `spread` into a new calculated number, keeping this out of `calculation`/`calculation_input` scope) and a Market Context panel (SOFR + HY OAS shown with source/as-of date). `security.figi` got its first unique index (`0005`), alongside new `fred_series_registry`/`fred_observation` tables (PLAN.md §4.5). Two real bugs were found and fixed during this milestone: `tests/integration/conftest.py`'s `db_engine` fixture had the same Supabase pgbouncer/psycopg3 prepared-statement incompatibility found (and fixed in `app/db/session.py`) in Milestone 4, but independently, since it builds its own separate engine — now fixed there too. And OpenFIGI's unauthenticated tier hit a real live 429 running its four live tests back-to-back; fixed by throttling the test client to 6s between requests and wiring `X-OPENFIGI-APIKEY` through automatically whenever `OPENFIGI_API_KEY` is configured. 166 backend tests pass (up from 134 in Milestone 4) and 29 frontend tests pass, alongside ruff/black/mypy/eslint/prettier/tsc/frontend production build/`pre-commit run --all-files`. |
| **Last Updated** | 2026-08-06 |
| **Current Git Branch** | main |
| **Latest Commit** | _(recorded in a follow-up docs commit, per established pattern — see `BUILD_LOG.md`)_ |
| **Next Milestone** | Milestone 6 — Issuer detail page + Capital Structure page/model (§18 step 6) |
...
**Milestones 1 through 5 are complete.** The provenance spine, raw-response
store, entitlement engine, and three real provider adapters (SEC EDGAR,
OpenFIGI, FRED) are implemented, tested, and manually verified end-to-end
against the live, shared Supabase project. Credit Universe — the post-login
landing page, powered entirely by the canonical domain model — now shows
real SEC-sourced aggregate debt, five real specific Apple bonds (via
OpenFIGI), ten clearly-tagged synthetic loan positions, a live
FRED-sourced benchmark rate per matching row, and a Market Context panel.
**Milestone 6 (Issuer detail page + Capital Structure page/model, §18 step
6) has not started**, awaiting explicit approval to begin. Do not begin
Milestone 6 until it is explicitly approved.
