...
| Field | Value |
|---|---|
| **Overall Progress** | ~25% — Milestone 4 of 16 complete |
| **Current Milestone** | Milestone 4 — Credit Universe initial page, backed by seeded canonical securities (complete) |
| **Current Status** | Milestone 4 is complete. The first usable Credit Universe screen is live end-to-end against the real, shared Supabase project and is now the post-login landing page. Backend: `security` domain object/ORM model (PLAN.md §4.5) got its first migration (`0004`, alongside new `is_synthetic`/`synthetic_reason` columns on `issuer`); `core/freshness.py` computes `live`/`cached`/`stale` at read time per provider TTL policy (never persisted, §16); `security_repository.list_credit_universe` joins security+issuer+provenance in one query with filter/sort/pagination (no N+1, built for thousands of rows per PLAN.md's UX requirement even though the seed set is small); `credit_universe_service` applies `policy_check` per row and assembles the response; `GET /api/credit-universe` is a thin route. Two real data sources are seeded and coexist, clearly distinguished: one real SEC EDGAR-sourced bond (Apple Inc., aggregate long-term debt from XBRL company-facts — deliberately left without CUSIP/maturity/coupon since the simple companyfacts API only exposes issuer-level aggregates, not per-instrument dimensional data; fabricating those fields was rejected, see TD-008) and ten synthetic leveraged-loan positions across 8 fictional issuers, tagged `SYNTHETIC_DEMO_DATA` in both the data model and the UI (ADR-008). Frontend: TanStack Table v8 (`DataTable`) renders a 9-column, sortable, paginated grid with `ProvenanceBadge`/`SyntheticDataBadge` on every row; filters (search, instrument type, real/synthetic toggle) are URL-persisted via `useSearchParams`; loading/empty/error states are all implemented and manually verified in-browser, not just asserted in tests. A genuine keystroke-loss bug in the search box (driving a controlled input directly from URL state loses characters mid-typing) was found via manual browser testing and fixed with a debounce hook, per CLAUDE.md's requirement to actually use the UI before reporting completion. A second real, live-reproduced bug was found during full-suite verification and fixed at the root: Supabase's pooled connection string uses pgbouncer in transaction-pooling mode, which is incompatible with psycopg3's default server-side prepared-statement caching — surfaced as intermittent `InvalidSqlStatementName`/`DuplicatePreparedStatement` errors on both the test suite and the live running server. Fixed by disabling server-side prepare (`prepare_threshold=None`) in `app/db/session.py`; confirmed via three consecutive clean full test-suite runs and a live re-check of the running server. 134 backend tests pass (up from 91 in Milestone 3) and 26 frontend tests pass, alongside ruff/black/mypy/eslint/prettier/tsc/frontend production build/`pre-commit run --all-files`. |
| **Last Updated** | 2026-08-06 |
| **Current Git Branch** | main |
| **Latest Commit** | _(recorded in a follow-up docs commit, per established pattern — see `BUILD_LOG.md`)_ |
| **Next Milestone** | Milestone 5 — OpenFIGI + FRED adapters (§18 step 5) |
...
**Milestones 1 through 4 are complete.** The provenance spine, raw-response
store, entitlement engine, first real provider adapter (SEC EDGAR), and now
the first usable Credit Universe screen are implemented, tested, and (for
Milestone 4) manually verified in-browser end-to-end against the live,
shared Supabase project. Credit Universe is the post-login landing page,
powered entirely by the canonical domain model, showing one real SEC-sourced
bond alongside ten clearly-tagged synthetic loan positions, each carrying
full provenance. **Milestone 5 (OpenFIGI + FRED adapters, §18 step 5) has
not started**, awaiting explicit approval to begin. Do not begin Milestone 5
