...
| **Current Milestone** | Milestone 7.5.3 (Historical Discovery Coverage Repair) — in progress, paused before the historical backfill itself, pending user approval of the AI budget |
| **Current Status** | Milestone 7.5.2 is complete (see prior entries). Milestone 7.5.3 began the 2026-01-01→2026-08-06 historical repair but three live attempts (2026-08-09) each ended in a real incident rather than completion: two runs stalled for 4+ hours on a CourtListener `Retry-After` defect (`ThrottledHttpClient` trusted an uncapped `time.sleep()` duration), and once that was root-caused via `py-spy` and fixed, the user paused the milestone entirely to require AI cost control before any further live run — this project's Anthropic spend had no per-call observability and always used Sonnet regardless of task complexity. Built and tested in response: `app/ai/model_router.py` (deterministic floor → Haiku → Sonnet escalation, with definitive/high-impact categories — Chapter 11, bankruptcy, plan-confirmed — routed straight to Sonnet, never Haiku, after live quality validation caught Haiku confidently misclassifying a real Chapter-11 third-party-attribution case), a new `ai_call_log` table (migration `0014`) with full per-call cost/token/latency observability, hard per-run budgets (`max_ai_calls`/`max_ai_cost_usd`/`max_sonnet_calls`, enforced centrally before every provider call), a zero-AI discovery-only mode, and pre-run cost estimation. Live quality validation (7 real production alerts re-reviewed) also caught and fixed a Haiku-specific bug: it wraps JSON responses in markdown code fences, which would have silently failed 100% of Haiku calls. 35 new tests added, all passing (418/418 backend tests total); ruff/black/mypy clean; migration verified live with zero drift. The historical backfill itself has not yet been (re)run to completion — awaiting explicit approval of the AI budget before restarting. Full detail in `BUILD_LOG.md`. |
| **Last Updated** | 2026-08-09 |
| **Current Git Branch** | main |
| **Latest Commit** | `(pending — see BUILD_LOG.md)` |
| **Next Milestone** | Resume Milestone 7.5.3's historical backfill once the user approves an AI budget; Milestone 7.5.4/7.6/Milestone 8 not to begin until explicitly approved |
...
| 7.5.3 | Historical Discovery Coverage Repair (inserted, in progress) | In Progress — paused before the historical backfill, pending user approval of the AI budget | `(pending)` | — | Re-runs the 2026-01-01→2026-08-06 historical discovery window with TD-014's corrected SEC full-text-search `forms` behavior active. Three live attempts (Aug 9) surfaced two real production incidents rather than completing: (1) a CourtListener `Retry-After` defect that stalled the whole batch for 4+ hours on an uncapped `time.sleep()` — root-caused via `py-spy`, fixed (RFC 7231-correct parsing + a hard wait ceiling); (2) unbounded, unobserved Anthropic spend risk — the user paused the milestone to require AI cost control, observability, and Haiku/Sonnet model routing before any further backfill. That control layer (`app.ai.model_router`, `ai_call_log` table/migration `0014`, hard per-run budgets, zero-AI mode, pre-run cost estimation) is now built and tested (35 new tests, 418/418 backend tests passing); live quality validation against 7 real production alerts caught and fixed a Haiku markdown-fence parsing bug and corrected the routing policy so definitive/high-impact categories (Chapter 11, bankruptcy, plan-confirmed) always go straight to Sonnet, never through Haiku. The Jan–Aug historical backfill itself has not yet been (re)run to completion — awaiting explicit user approval of the AI budget before restarting. Bitcoin Depot Inc. and GoHealth, Inc. were confirmed discovered during the partial runs; FAT Brands and Inotiv not yet seen. Full detail in `BUILD_LOG.md`. |
