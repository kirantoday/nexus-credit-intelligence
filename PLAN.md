...
| **Latest Commit** | 0e02096 — Milestone 10B: Research Documents + Supabase Storage |
...
| TD-030 | 2026-08-13 incident (see `BUILD_LOG.md`): `run_market_discovery.py`'s SEC discovery client was fixed to retry transient 5xx (`retry_on_status={500,502,503,504}, max_retries=2`, reusing the CourtListener/TD-020 pattern), but other SEC EDGAR call sites (`run_overnight_filing_monitor.py`, `seed_research_universes.py`, the production-disabled manual-trigger route in `filing_monitor_api_service.py`) still construct `ThrottledHttpClient` with no retry configuration | Low | Apply the same retry configuration to the other SEC call sites if any of them proves similarly exposed to `efts.sec.gov`/EDGAR transient 5xx in practice — not fixed preemptively since only the nightly discovery path was actually implicated | Open (deferred by design, `backend/app/scripts/run_market_discovery.py` vs. siblings) |
| TD-031 | A `market_discovery_run`/`filing_monitor_run` completing with `status=completed_with_errors` (by design, so one transient per-query failure never strands the watermark) still causes the wrapping script to exit non-zero, which Railway's cron-job monitor reports as a full "Crashed" deployment/email — operationally noisy and misleading for a run that was mostly successful and durably committed real data (live-caught 2026-08-13, see `BUILD_LOG.md`) | Low | Consider a distinct exit code or Railway alerting rule for `completed_with_errors` vs. a genuine unhandled crash, if the false-alarm rate becomes a real operational nuisance — not changed now per explicit instruction against a speculative change to get one run through | Open (deferred by design) |
| TD-032 | `tests/integration/test_morning_brief_service.py::test_no_material_changes_when_nothing_new` asserts `new_developments == []` against the live shared `nexus` schema's real "today" state with no isolation from concurrent real research-cycle activity — passed for the project's entire history until the 2026-08-13 incident's own approved same-day recovery run created real alerts that violated its unstated assumption (not a regression from that incident's code fix; the test's own comment already flagged the fragility) | Low | Scope the test more defensively (e.g. filter to synthetic/fixture-only issuer ids, or assert only on newly-seeded rows) rather than asserting emptiness against an uncontrolled live table | Open (discovered 2026-08-13, not yet fixed — out of scope for that incident) |
...
|---|---|---|---|
| KI-004 | ~~`nexus-nightly-10pm-edt` reported "Crashed" for the 2026-08-12 nightly run; Morning Research Brief stuck at `latest_research_day: 2026-08-11`.~~ **Resolved 2026-08-13.** Root cause: an unretried, transient SEC EDGAR full-text-search `500` on 1 of 39 queries left `status=completed_with_errors`, which stalled the daily-run watermark and caused a non-zero exit Railway reported as a crash — not an application/database/config failure, and unrelated to 10A/10B/Universal Search. Fixed by extending the existing, already-proven `ThrottledHttpClient` retry mechanism (CourtListener's 429 pattern) to SEC's transient-5xx set (commit `2924f3e`). Recovered via a plain `--mode delta` production rerun (not backfill — `research_day` derives from `window_start_date`, and `get_latest_successful_daily_run` excludes `backfill` mode by design, so only delta's watermark-resume behavior could correctly land the `2026-08-12` label). Verified live: production API and browser (`/research-brief`) both confirm `latest_research_day: 2026-08-12` / `preceding_research_day: 2026-08-11`; Aug 11 data byte-identical to pre-incident; no duplicate alerts/evidence/filings across three run attempts (idempotent dedup proven); Watchlists/Research Universes/Universal Search/Research Notes/Research Documents all healthy; both nightly cron services redeployed with the fix and on track for their next scheduled runs. See `BUILD_LOG.md` 2026-08-13 entry for full detail, including new TD-030/031/032. | Morning Research Brief was one research day stale in production; no data was lost. | **Resolved** |
