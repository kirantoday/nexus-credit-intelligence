...
| TD-019 | The backend runs as a single Uvicorn worker (`backend/railway.toml`/`Dockerfile`, no `--workers` flag) with no async offloading for large synchronous JSON response serialization. Live-caught during Milestone 7.5.2's correction: an uncapped `GET /api/morning-brief` response against a genuinely large period (a first-ever brief view, 621 historical-intelligence issuers, 3.5MB payload) intermittently caused the *next* request on the same reused connection (`POST /api/morning-brief/view`) to receive a `503` from Railway's edge — reproduced consistently via real browser page loads across three separate tabs, never reproducible via curl or a same-page manual `fetch()` call issued after the page had already settled, which points at response-serialization time on the single worker's event loop rather than a logic bug (the endpoint returns correctly, every time, under direct/manual testing). Mitigated for this specific endpoint by capping the number of issuer-grouped developments actually returned (`_ISSUER_DISPLAY_CAP = 100`, true counts unaffected), cutting the same real payload to ~730KB — not a general fix for the underlying single-worker/sync-serialization exposure, which could recur on any sufficiently large future response | Medium | Add `--workers N` (or switch large-list endpoints to streaming responses) once a genuinely large-payload endpoint needs it again; in the meantime, cap response size at the service layer for any endpoint whose data volume scales with real production data rather than a fixed page size | Open (mitigated for this endpoint, not resolved architecturally — `backend/app/services/morning_brief_service.py`) |
